...
function AddVisit() {
  ...
  const submit = async () => {
    ...
    var encrypted = await axios
      .post('http://localhost:5002/api/utils/encrypt', {
        user: username,
        password: password,
    ...
    await contractman.methods
      .addVisit(patId, address, encrypted, signed)
      .send({ from: address }); //FOR WRITING
