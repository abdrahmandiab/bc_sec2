...
const contractAddr = "0x6e080478A3100818E56c486C8EfBdD200c389D12";
...
function Home() {
  ...
  const submit = async () => {
    ...
    console.log("Awaiting Encryption from Server!")
    const encrypted = await axios({
      method: 'post',
      url: 'http://localhost:5000/api/utils/encrypt',
      headers: {}, 
      data: {
        user: username,
        password: password,
    ...
    console.log("Encryption: "+encrypted) //await  .toString(window.CryptoJS.enc.Utf8)
    ...
    console.log("Patients before: " + num_patients);
